// The Setlist category's own renderer: one dataset's Set List picker, a
// filterable/searchable/drag-and-drop-able table of a Set List's 128 song
// slots, and the per-slot General (Name/Color/Volume)/Comment editors --
// createPane()'s shell (pane.js) mounts createSetlistPanel() below as one of
// its category renderers, a peer of pane.js's own createLibraryPanels() (the
...
// a peer of pane.js's own createLibraryPanels() -- both are just "renderers"
...
function createSetlistPanel(
  ...
  // Same technique as pane.js's own shared scrollRowBelowHeader() (used by
  // pane-program-editor.js/pane-combi-editor.js) -- this table has a sticky
  // <thead> too, so a plain scrollIntoView({block: "center"}) can leave a
  // row still partly hidden under it, especially near the top of the list.
  // Kept as its own small copy here rather than switched over to that shared
  // one -- this predates it, and scrolls a DIFFERENT container class
  // (`.entries-scroll` vs `.library-body`), so per CLAUDE.md's duplicate-
  // code norm this is flagged to the project rather than unilaterally
  // merged.
  ...
  // row Set List reference (pane-program-editor.js's buildUsageRow()) is
  // clicked -- selects the target Set List first if this pane isn't already
  // showing it, then opens+scrolls to that exact slot, same as clicking the
  // row directly. Mirrors pane.js's own createLibraryPanels().jumpToEntry(),
  // the Program/Combi equivalent of this same "jump and land on the row"
  // pattern.
